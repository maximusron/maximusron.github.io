...
![alt-text-1](FinalSubmission0.png "title-1") ![alt-text-2](FinalSubmission1.png "title-2")
...
|  |   |
| :-: | :-: |
| __Cppyy__ : An automatic, run-time, Python\-C\+\+ bindings generator <br> __Cling__ is used in backend since an interactive C\+\+ interpreter provides a runtime exec approach to C\+\+ code <br>__Numba__ JIT compiler that translates Python and NumPy code into fast machine code| ![](cppyy_arch.png) |
...
![alt-text-1](FinalSubmission4.png "title-1") ![alt-text-2](FinalSubmission5.png "title-2")
...
This is a important step to addressing the challenges on the avenues of adding efficient reference type detection and handling, as well as extending the capabilities of the current template functions supported.
...
__Typing__ : Numba core has a type inference algorithm which assigns a nb\_type for a variable
...
We utilise the runtime numba compilation process to lower C\+\+ code cppdef’ed in PythonHow? 
...
![alt-text-1](FinalSubmission19.jpg "title-1") ![alt-text-2](FinalSubmission20.png "title-2")
...
Templated class args like Eigen are resolved to C\+\+ types and successfully matches the CPPOverload

The Numba typeinfer of the Eigen metaclass is refactored into the C\+\+ expression and handled in numba2cpp

Able to handle the templated class Eigen::Matrix< Scalar\_\, Rows\_\, Cols\_\, Options\_\, MaxRows\_\, MaxCols\_ >

using Eigen::Dynamic so the dispatcher typeinfer :
